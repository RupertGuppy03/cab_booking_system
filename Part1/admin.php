...
/*
  ...
  Description: Server-side handler for the CabsOnline admin panel. Responds to two
               types of POST requests from admin.js — booking searches and taxi
               assignment updates. For searches, queries the database for a specific
               booking by BRN or returns all unassigned bookings with a pickup time
               within 2 hours of the current time. For assignments, updates the status
               of a booking from unassigned to assigned and returns a confirmation message.
  ...
  Functions:
    ...
    - handle_assign():  Receives a BRN from admin.js and updates the matching record's
                        status from unassigned to assigned in the database. Returns a
                        plain text confirmation message containing the BRN.
*/
